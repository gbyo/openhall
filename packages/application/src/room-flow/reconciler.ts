...
import type { RoomFlowRepository, FlowReservationRow } from './ports.js';
...
/**
 * Durable room-flow worker. The database is the work source: every
 * action re-reads under the pass row lock (then the room-flow lock),
...
export class DestinationFlowReconciler {
  ...
  private async runOneForTenant(tenantId: string, now: Temporal.Instant): Promise<boolean> {
    ...
    const roomIds = await runner.run(tenantId, (context) =>
      this.dependencies.flow.listQueuedRoomIds(context, QUEUE_SCAN_LIMIT),
    );
    for (const roomId of roomIds) {
      if ((await this.tryPromoteRoom(tenantId, roomId, now)) === 'done') return true;
    }
  ...
  private async handleStaleReady(
    ...
    return runner.run(tenantId, async (context) => {
      ...
      const waiting = await flow.countActiveQueueEntries(context, pass.destinationRoomId, pass.id);
      if (waiting > 0) {
  ...
  private async tryPromoteRoom(
    ...
    const approx = await runner.run(tenantId, (context) =>
      (async () => {
        const config = await flow.loadRoomConfig(context, roomId);
        if (config === null) return null;
        const consuming = await flow.countConsumingReservations(context, roomId, now);
    ...
    return runner.run(tenantId, async (context) => {
      ...
      const head = await flow.loadQueueHead(context, roomId);
      ...
      await flow.acquireRoomLock(context, roomFlowLockKey(tenantId, roomId));
      const config = await flow.loadRoomConfig(context, roomId);
      ...
      const room = await passes.loadRoom(context, pass.destinationRoomId);
      const decided = await evaluateAndPersistPolicy(context, policy, {
        pass: {
          ...
          originRoomId: pass.originRoomId,
        },
        placement: current,
        at: now,
        stage: 'reevaluation',
