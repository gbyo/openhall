...
export function toPolicyRuleView(row: PolicyRuleRecord): PolicyRuleView {
  return {
    ...
    scope: {
      ...
      destinationCategoryId: row.scopeDestinationCategoryId,
    },
...
function canonicalWrite(body: PolicyWriteBody): PolicyRuleWrite {
  ...
  const { organizationId, sectionId, destinationId, destinationCategoryId } = body.scope;
  let scopeOrganizationId: string | null = null;
  let scopeSectionId: string | null = null;
  let scopeDestinationId: string | null = null;
  let scopeDestinationCategoryId: string | null = null;
  if (scopeKind === 'organization') {
    ...
    if (
      typeof destinationCategoryId !== 'string' ||
      organizationId !== null ||
      sectionId !== null ||
      destinationId !== null
    ) {
    ...
    scopeDestinationCategoryId = destinationCategoryId;
  }
 ...
 * destination-category scope must belong to it and not be archived.
 */
async function assertScopeValid(
  ...
  if (write.scopeKind === 'destination_category') {
    const category = await dependencies.categories.loadById(
      context,
      write.scopeDestinationCategoryId ?? '',
    );
    if (
    ...
    async (context, current, now) => {
      ...
      await assertScopeValid(
        ...
        input.principal.tenantId,
        {
          ...
          scopeDestinationCategoryId: current.scopeDestinationCategoryId,
