...
export async function getOwnQueueStatus(
  ...
  const loaded = await dependencies.runner.run(principal.tenantId, async (context) => {
    ...
    const position = await dependencies.flow.queuePosition(context, pass.destinationId, entry.id);
...
/**
 * GET /api/v1/destinations/:destinationId/station — minimized operational
...
export async function getStationView(
  ...
  const aggregates: StationAggregates | null = await dependencies.runner.run(
    ...
    async (context) => {
      ...
      return dependencies.flow.loadStationAggregates(context, destinationId, at);
    },
  );
  if (aggregates === null) {
  ...
  return {
    destination: {
      ...
      serviceType: aggregates.serviceType,
