...
import type { DestinationFlowRepository } from './ports.js';
...
export interface AllocationResult {
  ...
  readonly reasonCode: 'destination_unavailable' | 'destination_capacity_full' | null;
}
...
/**
 * Reusable destination-flow allocator. Runs inside the caller's existing
 ...
 */
export async function allocateDestinationFlow(
  context: TenantTransactionContext,
  ...
  await flow.acquireDestinationLock(
    context,
    destinationFlowLockKey(pass.tenantId, pass.destinationId),
  );

  const denyWithReason = async (
    reasonCode: 'destination_unavailable' | 'destination_capacity_full',
  ): Promise<AllocationResult> => {
    try {
  ...
  if (config.status === 'closed' || config.status === 'archived') {
    return denyWithReason('destination_unavailable');
  }
  ...
  return denyWithReason('destination_capacity_full');
}
