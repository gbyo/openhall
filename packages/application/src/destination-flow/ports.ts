...
/**
 * Purpose-built destination-flow persistence port. Methods are use-case
 * shaped; there is no generic SQL escape hatch. Capacity mutations must run
 * under the destination-flow advisory lock with the pass row already locked.
 */
export interface DestinationFlowRepository {
  /** All tenant ids, for per-tenant worker sweeps. No tenant scoping needed. */
  listTenantIds(): Promise<string[]>;
  acquireDestinationLock(context: TenantTransactionContext, lockKey: bigint): Promise<void>;
  ...
  listQueuedDestinationIds(context: TenantTransactionContext, limit: number): Promise<string[]>;
...
export interface StationAggregates {
  readonly config: DestinationFlowConfig;
