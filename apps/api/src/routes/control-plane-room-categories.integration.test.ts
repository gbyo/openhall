...
async function openRoom(admin: SessionFixture, roomId: string): Promise<void> {
...
async function createRoom(
  ...
  return { response, room: response.json<{ room: { id: string } }>().room };
}
...
describe('room-category administration', () => {
  it('creates, reads, updates, and archives a category with ETag concurrency', async () => {
    ...
    expect(created.etag).toBe(`"room-category:${created.category.id}:1"`);
    ...
    const updated = await app.inject({
      method: 'PUT',
      url: `/api/v1/room-categories/${created.category.id}`,
    ...
    expect(requiredEtag(updated)).toBe(`"room-category:${created.category.id}:2"`);
    ...
    const archived = await app.inject({
      method: 'POST',
      url: `/api/v1/room-categories/${created.category.id}/archive`,
    ...
    // Archived history stays readable; renames never rewrite rooms.
  ...
  it('rejects duplicate active names case-insensitively but reuses archived names', async () => {
    ...
    expect(duplicate.response.json<{ code: string }>().code).toBe('room_category_exists');
    ...
    const archived = await app.inject({
      method: 'POST',
      url: `/api/v1/room-categories/${first.category.id}/archive`,
  ...
  it('rejects unknown icons and conceals cross-tenant categories', async () => {
    ...
    const concealed = await app.inject({
      method: 'GET',
      url: `/api/v1/room-categories/${foreign.category.id}`,
      headers: authHeaders(requireAdmin()),
    });
    expect(concealed.statusCode).toBe(404);
    expect(concealed.json<{ code: string }>().code).toBe('room_category_not_found');
  });

  it('requires room.manage and replays idempotent creates', async () => {
  ...
  it('blocks archive while non-archived rooms reference the category', async () => {
    ...
    const blocked = await app.inject({
      method: 'POST',
      url: `/api/v1/room-categories/${category.category.id}/archive`,
      headers: authHeaders(requireAdmin(), randomUUID(), category.etag),
    });
    expect(blocked.statusCode).toBe(409);
    expect(blocked.json<{ code: string }>().code).toBe('room_category_in_use');

    // Archiving the room clears the guard; the category archives cleanly.
    const detail = await app.inject({
      method: 'GET',
      url: `/api/v1/rooms/${room.room.id}`,
      headers: authHeaders(requireAdmin()),
    });
    const archivedRoom = await app.inject({
      method: 'POST',
      url: `/api/v1/rooms/${room.room.id}/archive`,
      headers: authHeaders(requireAdmin(), randomUUID(), requiredEtag(detail)),
    });
    expect(archivedRoom.statusCode).toBe(200);

    const refreshed = await app.inject({
      method: 'GET',
      url: `/api/v1/room-categories/${category.category.id}`,
      headers: authHeaders(requireAdmin()),
    });
  ...
  it('validates room category references on create and update', async () => {
    const missing = await createRoom(requireAdmin(), schoolA, randomUUID());
...
describe('student room catalog and request gating', () => {
  it('serves only eligible categories and rooms to students', async () => {
    ...
    const open = await createRoom(admin, schoolA, primary.category.id, {
      name: 'Open Restroom',
      checkInMode: 'optional',
    });
    expect(open.response.statusCode).toBe(201);
    // Open the room so student requests can target it.
    await openRoom(admin, open.room.id);
    const gated = await createRoom(admin, schoolA, primary.category.id, {
      name: 'Staff Only Restroom',
      studentSelfRequestable: false,
    });
    expect(gated.response.statusCode).toBe(201);
    const more = await createRoom(admin, schoolA, secondary.category.id, {
      name: 'Back Room',
    });
    expect(more.response.statusCode).toBe(201);
    await openRoom(admin, more.room.id);
    const secret = await createRoom(admin, schoolA, hidden.category.id, {
      name: 'Secret Room',
    });
    ...
    const catalog = await app.inject({
      method: 'GET',
      url: `/api/v1/me/organizations/${schoolA}/student-room-catalog`,
    ...
    expect(primaryEntry?.rooms.map((d) => d.name)).toEqual(['Open Restroom']);
    expect(primaryEntry?.iconKey).toBe('restroom');
    expect(primaryEntry?.pickerMode).toBe('list');
    const secondaryEntry = body.categories.find((c) => c.name === 'Catalog Secondary');
    expect(secondaryEntry?.rooms.map((d) => d.name)).toEqual(['Back Room']);
    expect(empty).toBeDefined();

    // Flat member catalog is unchanged in shape (plus categoryId) and still
    // lists every open room, requestable or not.
    ...
    const flatEntries = flat.json<{
      rooms: { name: string; categoryId: string }[];
    }>().rooms;
    expect(flatEntries.map((d) => d.name)).toEqual(
  ...
  it('gates student self-service while staff creation stays open', async () => {
    ...
    const hiddenRoom = await createRoom(admin, schoolA, hidden.category.id, {
      name: 'Hidden Appt',
    });
    await openRoom(admin, hiddenRoom.room.id);
    // Staff creates directly against the hidden room.
    const staffCreated = await app.inject({
      method: 'POST',
      url: `/api/v1/students/${student.personId}/passes`,
      headers: authHeaders(admin, randomUUID()),
      payload: { destinationRoomId: hiddenRoom.room.id },
    });
    expect(staffCreated.statusCode).toBe(201);

    // The student cannot self-request the hidden room.
    ...
    expect(denied.json<{ code: string }>().code).toBe('room_unavailable');

    // Nor a room flagged off for self-service.
    const offCat = await createCategory(admin, schoolA, { name: 'Gate Off' });
    const offRoom = await createRoom(admin, schoolA, offCat.category.id, {
      name: 'Off Room',
      studentSelfRequestable: false,
    });
    const deniedOff = await app.inject({
      ...
      url: '/api/v1/me/passes',
      headers: authHeaders(student, randomUUID()),
      payload: { destinationRoomId: offRoom.room.id },
    });
  ...
  it('conceals the student catalog from students of other schools', async () => {
    const other = await app.inject({
      method: 'GET',
      url: `/api/v1/me/organizations/${schoolB}/student-room-catalog`,
      headers: authHeaders(requireStudent()),
    });
