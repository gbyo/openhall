...
async function makeRoom(input: {
  ...
  return insertReturningId(
    `INSERT INTO room (tenant_id, organization_id, category_id, student_self_requestable, name, capacity, queue_enabled, check_in_mode, status) VALUES ($1, $2, $3, true, $4, $5, $6, 'optional', 'open') RETURNING id`,
    [
      tenantA,
      schoolA,
      categoryId,
...
beforeAll(async () => {
  ...
  reconciler = new DestinationFlowReconciler({
    ...
    flow: new PostgresRoomFlowRepository(handle.database),
...
describe('allocator capacity and queueing', () => {
  it('fills exactly one slot under 20 concurrent requests', async () => {
    ...
    const roomId = await makeRoom({ capacity: 1, queueEnabled: true });
    ...
    const consuming = (
      await pool.query<{ count: string }>(
        `SELECT count(*) AS count FROM room_reservation
         WHERE room_id = $1 AND released_at IS NULL
    ...
    const deadlines = (
      await pool.query<{ flow_expires_at: Date; entered_at: Date }>(
        `SELECT flow_expires_at, entered_at FROM queue_entry WHERE room_id = $1`,
        [roomId],
      )
    ).rows;
...
describe('reconciler expiry', () => {
  it('expires a queued attempt past its flow deadline', async () => {
    ...
    await requestPass(holder, roomId);
    const waiter = await makeStudent(tenantA, schoolA, 'ExpiryWaiter');
    const queued = await requestPass(waiter, roomId);
  ...
  it('expires a missed ready offer when nobody waits', async () => {
    ...
    const ready = await requestPass(student, roomId);
    await pool.query(
  ...
  it('requeues a missed ready offer behind the waiter with the same deadline', async () => {
    ...
    const ready = await requestPass(holder, roomId);
    const waiter = await makeStudent(tenantA, schoolA, 'RequeueWaiter');
    const queued = await requestPass(waiter, roomId);
    expect(queued.pass.lifecycleState).toBe('queued');
    const originalFlow = (
      await pool.query<{ flow_expires_at: Date }>(
        `SELECT flow_expires_at FROM room_reservation WHERE pass_id = $1`,
        [ready.pass.id],
      )
...
describe('reconciler promotion', () => {
  it('promotes the head in FIFO order when capacity frees', async () => {
    ...
    const held = await requestPass(holder, roomId);
    const second = await makeStudent(tenantA, schoolA, 'PromoteSecond');
    const queuedSecond = await requestPass(second, roomId);
    const third = await makeStudent(tenantA, schoolA, 'PromoteThird');
    const queuedThird = await requestPass(third, roomId);
  ...
  it('terminalizes queued and ready flows when the room closes', async () => {
    await clearFlowRules();
    const roomId = await makeRoom({ capacity: 1, queueEnabled: true });
    ...
    const queued = await requestPass(waiter, roomId);
    await pool.query(`UPDATE room SET status = 'closed' WHERE id = $1`, [roomId]);
    ...
    expect(reasons).toEqual(['room_unavailable', 'room_unavailable']);
  });
...
describe('reconciler safety and races', () => {
  ...
  it('orders depart-then-expire and expire-then-depart deterministically', async () => {
    ...
    const readyEarly = await requestPass(early, roomId);
    await pool.query(
      `UPDATE room_reservation
       SET reserved_at = statement_timestamp() - interval '120 seconds',
           ready_expires_at = statement_timestamp() - interval '60 seconds'
    ...
    const readyLate = await requestPass(late, roomId);
    await pool.query(
      `UPDATE room_reservation
       SET reserved_at = statement_timestamp() - interval '120 seconds',
           ready_expires_at = statement_timestamp() - interval '60 seconds'
...
describe('reconciler fairness chain', () => {
  ...
  it('promotes A then B then C with missed claims requeued behind', async () => {
    ...
    const queuedA = await requestPass(studentA, roomId);
    const studentB = await makeStudent(tenantA, schoolA, 'ChainB');
    const queuedB = await requestPass(studentB, roomId);
    const studentC = await makeStudent(tenantA, schoolA, 'ChainC');
    const queuedC = await requestPass(studentC, roomId);
    ...
    const reservationC = (
      await pool.query<{ policy_evaluation_id: string }>(
        `SELECT policy_evaluation_id FROM room_reservation
         WHERE pass_id = $1 AND released_at IS NULL`,
        [queuedC.pass.id],
      )
    ).rows[0]?.policy_evaluation_id;
...
describe('allocator unlimited capacity', () => {
  it('grants ready to every policy-cleared pass without queue rows', async () => {
    await clearFlowRules();
    const roomId = await makeRoom({ capacity: null, queueEnabled: true });
    const students = await Promise.all(
    ...
    const queued = (
      await pool.query<{ count: string }>(
        `SELECT count(*) AS count FROM queue_entry WHERE room_id = $1`,
        [roomId],
      )
    ).rows[0];
...
describe('reconciler active-movement immunity', () => {
  it('leaves arrived and returning movement alone past every deadline', async () => {
    ...
    const readyFirst = await requestPass(first, roomId);
    const departed = await app.inject({
      method: 'POST',
...
describe('allocator queue fairness', () => {
  it('queues a newcomer behind waiters instead of granting a freed slot', async () => {
    ...
    const created = await requestPass(newcomer, roomId);
