...
/**
 * Assigned staff section for a room. Renders existing `room_staff`
 * authorization grants as the only source of truth — nothing is copied onto
 * the room row. Assignment controls appear only for accounts with the
 * existing `authorization.manage` capability; the server stays authoritative
 * on every mutation.
 */
export function RoomAssignedStaff({ roomId }: { roomId: string }) {
  ...
  const assigned = (grants.data?.grants ?? []).filter(
    (grant) => grant.role === 'room_staff' && grant.roomId === roomId && grant.status === 'active',
  );
  ...
  return (
    <FieldSet>
      <div className="flex items-center justify-between gap-2">
        <FieldLegend>Room staff</FieldLegend>
        {canManage ? (
          <Button
            variant="outline"
            size="sm"
      ...
      {!canManage ? (
        ...
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              Not every room needs staff — a restroom with none is completely valid.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : (
          ...
          {assigned.map((grant) => (
            <Item key={grant.id} variant="outline">
              ...
              <ItemContent>
                ...
                <ItemDescription>Room staff</ItemDescription>
              </ItemContent>
              <ItemActions>
                <Button
                  variant="ghost"
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Assign room staff</DialogTitle>
            <DialogDescription>
              They will receive room staff access for this room in Station.
            </DialogDescription>
          </DialogHeader>
          {people.isPending ? (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmingRemove
                ? `Remove ${confirmingRemove.personName} from this room?`
                : 'Remove assigned staff?'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              They will no longer have room staff access to this room.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
