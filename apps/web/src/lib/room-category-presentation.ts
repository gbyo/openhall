...
/**
 * Single centralized room-category presentation registry.
 *
...
export type RoomPickerMode = 'auto' | 'list' | 'search';

export function isRoomPickerMode(value: string): value is RoomPickerMode {
  return value === 'auto' || value === 'list' || value === 'search';
}

/** Rooms at or below this count render as a plain list under `auto`. */
export const ROOM_AUTO_LIST_THRESHOLD = 8;

/**
 * Resolve the effective student picker for a category. `auto` renders a
 * list below the threshold and search at or above it; explicit modes win.
 * Never branches on category names — only on the persisted picker mode.
 */
export function resolveRoomPickerMode(
  mode: string,
  roomCount: number,
  threshold: number = ROOM_AUTO_LIST_THRESHOLD,
): 'list' | 'search' {
  if (mode === 'list') return 'list';
  if (mode === 'search') return 'search';
  return roomCount <= threshold ? 'list' : 'search';
}

/**
 * Normalize free text for room search: lowercase, collapse whitespace,
 * strip ordinary punctuation. No fuzzy matching — substring only.
 */
...
/** Substring match for a room against teacher/name/number/class text. */
export function matchesRoomSearch(haystack: string, query: string): boolean {
  const normalized = normalizeRoomSearch(query);
  if (normalized.length === 0) return true;
  return normalizeRoomSearch(haystack).includes(normalized);
}
